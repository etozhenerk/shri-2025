...
test('TC-HY-010: Скриншот пустой страницы истории', async ({ actions, pages }) => {
...
test('TC-HY-008: Скриншот состояния страницы c заполненным списком', async ({ actions, pages }) => {
...
test('TC-HY-009: Скриншот состояния модального окна с деталями истории', async ({ actions, pages }) => {
    await actions.history.seedHistory(historyMock);
