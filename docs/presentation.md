...
Всего в проекте **150 тестов** (фактические замеры), распределенных по страницам и компонентам:

```mermaid
%%{init: {'theme':'base', 'themeVariables': { 'fontSize': '18px', 'fontFamily': 'Arial', 'primaryColor': '#ffffff', 'primaryTextColor': '#000000'}}}%%
graph TD
    A["📊 Всего тестов: 150"] --> B["🏠 Home Page<br/>13 тест-кейсов"]
A --> C["📄 Generate Page<br/>6 тест-кейсов"]
A --> D["📋 History Page<br/>10 тест-кейсов"]
A --> E["🎨 UI Components<br/>28 тестов"]
A --> F["♿ Accessibility страниц<br/>4 теста"]
A --> G["⚡ Юнит + Интеграционные + A11Y компонентов<br/>101 тест"]
    ...
    F --> F1["🟣 Страницы Home, Generate, History"]
    
    G --> G1["⚫ API: 18 тестов"]
    G --> G2["⚫ Utils: 62 теста"]
    G --> G3["⚫ Интеграционные: 12 тестов"]
...
Интеграционные тесты составляют **основу** нашего функционального покрытия (12 тестов от общего количества 150, но покрывают ключевую бизнес-логику). Этот выбор обоснован следующими факторами:
...
| Тип тестов | Количество | Время выполнения | Качество |
|------------|------------|------------------|----------|
| Юнит + Интеграционные + A11Y компонентов | 101 | 1.49 сек | ✅ 100% стабильность |
| E2E функциональные | 3 | 3.6 сек | ✅ Критические пути |
| Accessibility страниц | 4 | 2.5 сек | ✅ WCAG 2.1 AA |
| Скриншотные | 14 | 3.2 сек | ✅ Визуальная стабильность |
| UI компоненты (Storybook) | 28 | 5.4 сек | ✅ Comprehensive coverage |
| **Итого** | **150** | **~16 секунд** | **✅ Продакшн-готовое** |
...
**Юнит + Интеграционные + A11Y компонентов (101 тест - 1.49 сек):**
- API тесты: 18 тестов (analysis, report)
- Utils тесты: 62 теста (storage, formatDate, persist, analysis)
...
**E2E функциональные (3 теста - 3.6 сек):**
- Загрузка и анализ файла
- Генерация отчета
- Работа с историей

**Accessibility страниц (4 теста - 2.5 сек):**
- Проверка соответствия WCAG на всех страницах

**Скриншотные (14 тестов - 3.2 сек):**
- Снимки ключевых состояний интерфейса

**UI компоненты (28 тестов - 5.4 сек):**
...
npm run test:unit          # 101 тест - 1.49 сек
npm run test:functional    # 3 теста - 3.6 сек  
npm run test:screenshots   # 14 тестов - 3.2 сек
npm run test:e2e:a11y      # 4 теста - 2.5 сек
npm run test:storybook:snapshot  # 28 тестов - 5.4 сек

# Запуск всех основных тестов последовательно
npm run test              # 122 теста - 11.2 сек (без UI компонентов)

# Проверка стабильности тестов (flaky-тесты)
npm run test:flaky:all    # для проверки стабильности всех тестов
npm run test:flaky "TC-HP-001" 10  # конкретный тест 10 раз
```

### Ключевые выводы по производительности

**Фактические замеры показывают отличную производительность:**
- **Общее время**: 11.2 секунды для всех 122 основных тестов
- **Полный набор**: 16.6 секунд для всех 150 тестов включая UI компоненты
- **Скорость разработки**: мгновенный фидбек для разработчиков

**Стабильность тестов:**
- **100% стабильность** - все тесты проходят
- **Отсутствие flaky-тестов** - надежное выполнение
- **Практическое применение**: скрипты позволяют проверить любой тест на стабильность

**Распределение времени:**
- 67% времени (11.2 сек) - критические тесты (функциональность + скриншоты)
- 33% времени (5.4 сек) - UI компоненты (изоляция, отдельный запуск)
...
pie title Распределение тестов по типам (150 тестов)
    "Юнит+Интеграционные+A11Y (101)" : 101
    "UI компоненты (28)" : 28
    "Скриншотные (14)" : 14
