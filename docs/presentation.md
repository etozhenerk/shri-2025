...
Всего в проекте **109 тест-кейсов** (фактические замеры), распределенных по страницам и компонентам:

**⚠️ Примечание:** Диаграмма показывает основные группы тестов. В реальном проекте также выполняются A11Y тесты компонентов (входят в общую сумму 60 тестов Vitest), что объясняет разницу в цифрах диаграммы и общего итога.
...
graph LR
    ...
    subgraph "Юнит-тесты - 39 тестов"
        ...
        UT6[Utils: analysis.ts - 12 тестов]
    end
```
...
**Юнит + Интеграционные + A11Y компонентов (60 тестов - 1.42 сек):**
- API тесты: 8 тестов (analysis, report)
- Utils тесты: 31 тест (storage, formatDate, persist, analysis)
...
npm run test:unit          # 60 тестов - 1.42 сек
...
- **Полный набор**: 17 секунд для всех 109 тестов включая UI компоненты
- **Параллельное выполнение**: 327% CPU usage - эффективная утилизация ресурсов
