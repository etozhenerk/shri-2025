...
| **UI-Kit** |
| TC-UI-001 | Визуальные состояния компонентов | Визуальный (Компонент) | **Высокая** | Гарантирует, что все компоненты UI-кита (`packages/ui-kit`) выглядят корректно во всех своих состояниях (default, disabled, hover и т.д.). | 